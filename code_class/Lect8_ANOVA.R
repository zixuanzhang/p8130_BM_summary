...
#           Author: Cody Chiuzan; Date: Oct 2, 2018            #
################################################################
...
# five groups, each with different stimulants, and record insulin levels
...
# Re-shape the data
insulin <- c(ins1,ins2,ins3,ins4,ins5) # a single vector

index <- c(rep(1,length(ins1)),rep(2,length(ins2)),rep(3,length(ins3)),rep(4,length(ins4)),rep(5,length(ins5)))
...
tapply(insulin, ind, tmp_functn) # apply function to each combination of insulin and index (group)
